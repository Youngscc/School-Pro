...
def parser_option():
    ...
    parser.add_argument('--ensemble', type=str, default='average', choices=['average', 'maximum', 'scaling', 'scaling_unlabel', 'dropout', 'perturb', 'scaledinf', 'deepgini'])
...
def main():
    ...
    if opt.ensemble == 'average':
        opt.softmax = False
    elif opt.ensemble == 'scaling':
        model_e.scaling(val_loader, opt)
    elif opt.ensemble == 'scaling_unlabel':
        model_e.scaling_unlabel(val_loader, opt)
        opt.softmax = False
    elif opt.ensemble == 'dropout':
        opt.softmax = False
    elif opt.ensemble == 'perturb':
        opt.softmax = False
    
    model_e.eval()

    print('eval the ensemble model:')
    test(model_e, test_loader, opt)
