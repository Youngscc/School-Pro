...
def get_emnist_dataloaders(batch_size=128, num_workers=8, size=1.0):
    ...
    train_dataset = datasets.EMNIST(root=data_folder,
                                    split='mnist',
                                    download=True,
                                    train=True,
    ...
    test_dataset = datasets.EMNIST(root=data_folder,
                                 split='mnist',
                                 download=True,
                                 train=False,
    ...
    idx = np.arange(n_data)
    np.random.shuffle(idx)
    split = int(np.floor(0.01 * n_data))
    val_idx = idx[:split]
