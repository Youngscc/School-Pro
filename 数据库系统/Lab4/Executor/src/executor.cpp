...
static unsigned int getTableSize(const File &tableFile) {
    unsigned int ret = 0;
    auto file = tableFile;
    for (auto p:file) ret++;
    return ret;
...
bool NestedLoopJoinOperator::execute(int numAvailableBufPages,
    ...
    File l_file = leftTableFile, r_file = rightTableFile;
    TableSchema l_schema = leftTableSchema, r_schema = rightTableSchema;



    if (getTableSize(leftTableFile) <= getTableSize(rightTableFile)) ;
    else {
        l_file = rightTableFile, r_file = leftTableFile;
        l_schema = rightTableSchema, r_schema = leftTableSchema;
    }

    int buffer_cap = numAvailableBufPages - 1, buffer_p;
    Page* buffer_pages[buffer_cap];
    Page* r_page;

    for (auto l_it=l_file.begin(); l_it!=l_file.end();) {
        for (buffer_p = 0; buffer_p < buffer_cap && l_it != l_file.end(); ++l_it,++buffer_p) {
            numIOs++;
            numUsedBufPages++;
            bufMgr -> readPage(&l_file, (*l_it).page_number(), buffer_pages[buffer_p]);
        }

        vector<string> l_tuple, r_tuple;
        string raw_tuple;
        ...
        for (auto r_it = r_file.begin(); r_it!=r_file.end(); ++r_it) {
            ...
            for (auto rp_it = r_page->begin(); rp_it!=r_page->end(); ++rp_it) {
                didit(r_schema, *rp_it, r_tuple);
                for (int buffer_now=0; buffer_now<buffer_p; ++buffer_now) {
                    for (auto lp_it = buffer_pages[buffer_now]->begin(); lp_it != buffer_pages[buffer_now]->end(); ++lp_it) {
                        didit(l_schema, *lp_it, l_tuple);
                        bool succeess = true;
                        raw_tuple.clear();
                        for (int i=0; i<resultTableSchema.getAttrCount(); ++i) {
                            auto attr = resultTableSchema.getAttrName(i);
                            auto l_pd = l_schema.hasAttr(attr);
                            auto r_pd = r_schema.hasAttr(attr);
                            if (l_pd && r_pd) {
        ...
        for (int buffernow = 0; buffernow < buffer_p; ++buffernow) 
            bufMgr->unPinPage(&l_file, (*buffer_pages[buffernow]).page_number(), false);
    }
