 ...
 */
int builtin_cmd(char **argv) {
	if (!strcmp(argv[0], "quit")) exit(0); // 退出命令
	if (!strcmp(argv[0], "&")) return 1; //忽略单独的&
	if (!strcmp(argv[0], "jobs")) { // jobs指令
		listjobs(jobs);
		return 1;
	}
	if (!strcmp(argv[0], "bg") || !strcmp(argv[0], "fg")) { // fg/bg指令
		do_bgfg(argv);
		return 1;
	}
...
void sigchld_handler(int sig) 
{
	...
	pid_t pid;
	sigfillset(&mask); // 将所有信号添加到mask中

	while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) { // while循环尽可能回收所有的僵死进程
		//对导致退出的情况进行屏幕回显，WNOHANG|WUNTRACED 代表立即返回，如果等待集合中的子进程都没有停止或终止则返回为0，否则返回他的PID
		sigprocmask(SIG_BLOCK, &mask, &pre_mask);  //需要阻塞所有信号
		NewJob = getjobpid(jobs, pid);  //通过pid找进程
		if (WIFSTOPPED(status)) {//如果子进程停止引起waitpid函数返回
			NewJob->state = ST;
			printf("Job [%d] (%d) stopped by signal %d\n", NewJob->jid, NewJob->pid, WSTOPSIG(status));
		}
		else {
			if (WIFSIGNALED(status)) printf("Job [%d] (%d) terminated by signal %d\n", NewJob->jid, NewJob->pid, WTERMSIG(status));
			//子进程是因为一个未被捕获的信号终止的
			deletejob(jobs, pid);  //终止的进程直接回收
		}
		fflush(stdout);
		sigprocmask(SIG_SETMASK, &pre_mask, NULL); //解除阻塞
	}
	errno = olderrno;
    return;
}
...
void sigint_handler(int sig) 
{
	...
	int olderrno = errno;
	sigfillset(&mask);
	sigprocmask(SIG_BLOCK, &mask, &pre_mask);  //阻塞所有信号
	pid = fgpid(jobs);  //获取前台作业pid
	sigprocmask(SIG_SETMASK, &pre_mask, NULL); // 解除阻塞
	if (pid != 0) kill(-pid, SIGINT); // 判断是否为前台作业
	errno = olderrno;
    return;
}
